...
//ボタンのポストバックデータが送られてきたとき
    ...
    if($postdata == "confilm_ok"){
        //MySQLデータベースに接続する
        require 'dbconect.php';
        try{
            $sql = "SELECT DISTINCT a.id, a.name FROM user a, send_log b 
                WHERE a.LINE_id = :id AND a.id = b.to_id AND b.confilm_check = false AND b.datetime >= :min_datetime AND b.datetime <= :now_datetime";
            //プリペアドステートメントを作る
            $stm = $pdo->prepare($sql);
            //プレースホルダに値をバインドする
            $stm->bindValue(':id', $user_id, PDO::PARAM_STR);
            ...
            //ログを更新する
            $sql = "UPDATE send_log SET confilm_check = true WHERE to_id = :id AND datetime >= :min_datetime AND datetime <= :now_datetime";
            //プリペアドステートメントを作る
            $stm = $pdo->prepare($sql);
            //プレースホルダに値をバインドする
        ...
        try{
            $sql = "SELECT DISTINCT a.id, a.name FROM user a, send_log b 
                WHERE LINE_id = :id AND a.id = b.to_id AND b.confilm_check = false AND b.datetime >= :min_datetime AND b.datetime <= :now_datetime 
